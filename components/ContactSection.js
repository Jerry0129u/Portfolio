...
export function ContactSection() {
  return (
    <section>
      <div className="w-[375px] h-[472px] pt-4 flex-col flex gap-8 ">
        <div className="w-[343px] h-[344px] my-16 mx-4 flex flex-col gap-4">
          ...
          {contacts.map((call, index) => (
            <div key={index} className="flex flex-col gap-1 ">
              <div className="flex justify-center gap-4">
                <img src={`images/${call.image1}`} className="w-6 h-6"/>
                <p className="font-sans font-semibold text-base leading-7">{call.contact}</p>
                <img src={`images/${call.image2}`} className="w-6 h-6"/>
              </div>
            </div>
          ))}
        </div>
